...
import { api, getErrorMessage, type Algorithm } from '../lib/api'
import { DIFFICULTY_COLORS, LIST_COLORS, parseJson, ScoreBadge } from '../lib/ui'
        ...
        <div className="flex flex-wrap gap-2 items-center">
          ...
          {allLists.map(l => (
            <button key={l} onClick={() => setFilterList(filterList === l ? null : l)}
              className={`text-xs px-3 py-1 rounded-full border transition-colors ${filterList === l ? LIST_COLORS[l] || 'bg-zinc-700 text-zinc-200 border-zinc-600' : 'bg-zinc-900 text-zinc-500 border-zinc-800 hover:border-zinc-600'}`}>
              {l}
            </button>
          ))}
        </div>
          ...
          <div key={p.id} className="flex items-center justify-between bg-zinc-900 border border-zinc-800 rounded-lg px-4 py-3 hover:border-zinc-700 transition-colors group">
            <div className="flex items-center gap-3 min-w-0">
              ...
              {parseJson(p.lists).map(l => (
                <span key={l} className={`text-[10px] px-1.5 py-0.5 rounded border shrink-0 hidden md:inline ${LIST_COLORS[l] || 'bg-zinc-800 text-zinc-500 border-zinc-700'}`}>{l}</span>
              ))}
            </div>
